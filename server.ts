...
// ---------------- In-memory fallback seed data (kept for demo when DB is empty) ----------------
const initialPros: VerifiedPro[] = [
  ...
  // ... other pros omitted for brevity (kept in DB/seed)
];

// Fallback stores/drivers/orders omitted here — the server will prefer DB data when available.

// ---------------- Helpers: load from DB with fallback to in-memory ----------------
async function getPros(): Promise<VerifiedPro[]> {
  try {
    ...
    // Map Prisma model fields to VerifiedPro shape
    return rows.map(r => ({
      id: r.id,
      name: r.name,
      avatar: r.avatar || '',
...
app.post('/api/auth/register', async (req: Request, res: Response) => {
  ...
  try {
    ...
    res.json({ success: true, data: { token, user: { id: user.id, email: user.email, role: user.role, name: user.name } } });
  } catch (err) {
